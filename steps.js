...
const CustLogin = require('../pageobjects/1CustLogin.page.js');
const CustDD = require('../pageobjects/2CustDD.page.js');
const CustAcc = require('../pageobjects/3UserTrans.page.js')
...
Then (/^I login by clicking on the Customer Login button$/, async()=>{
    await CustLogin.Login();
});

Then (/^I click on my name$/, async()=>{
    await CustDD.dropdown();
});

Then (/^I click on the login button$/, async()=>{
    await CustDD.loginNow();
});

Then (/^I select the Account Number$/, async()=>{
    await CustAcc.accSelection();
});

Then (/^I click on the Transactions button$/, async()=>{
    await CustAcc.clickTransBtn();
});

// Then (/^I click on the Transactions button$/, async()=>{
//     await CustAcc.loginNow();
// });
